...
`NLE_Vote_Counts_2007-2025.csv.gz` by summing each candidate's votes across every locality —
...
| `City` | string | City / municipality, for the **municipal** offices (mayor, vice mayor, councilor). From **2016** on it is the canonical name off a scraped ballot (matching `city` in the vote-counts dataset). **2001–2013 come from COMELEC's List of Elected Candidates**, which prints the town — recovered for 2004–2013 (~93–96%) by matching each winner to that year's list on province + surname + given, then, since the 2004 list parses its province poorly, on surname + given nationwide accepting the town only when it lies in the winner's province (the inherited v8.5 source those cycles came from had no town). Blank for provincial and district offices (a governor has no city) and for the small tail of 2004–2013 winners no list entry matched. |
...
| Cycles | Source |
|---|---|
| **2016, 2019, 2022, 2025** | **Derived from the ballots** in `NLE_Vote_Counts_2007-2025.csv.gz`: votes are summed within the jurisdiction that elects each seat, and the top candidate(s) win. Fully reproducible and independently checkable. |
