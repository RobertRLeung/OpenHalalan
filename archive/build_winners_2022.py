...
from config import DEFAULT_COUNCILORS, PROCESSED, SOURCE_WINNERS, raw_dir, winners_csv
...
def load_2019_board_member_counts():
    ...
    """
    nle_df = pd.read_csv(SOURCE_WINNERS, low_memory=False)
...
def main():
    ...
    all_data = []
    csv_files = list(raw_dir(2022).rglob("*.csv"))
    ...
    # Winners are decided by VOTES, never by the `rank` column.
    #
    # COMELEC's `rank` is an ALPHABETICAL index, not a vote standing. This script used to
    # sort on it and take the top row, which silently selected the alphabetically-first
    # candidate in every race: the 2022 mayor of Manila came out as "ABAD, ONOFRE" (2,618
    # votes, rank 1) instead of "LACUNA, HONEY" (538,595 votes, rank 4).
    for position in ['MAYOR', 'VICE MAYOR', 'COUNCILOR']:
        position_df = df[df['standard_position'] == position].copy()

        # Councils can be elected by district (the City of Manila returns councilors in
        # each of its 6 council districts), so the seats must be filled per district, not
        # pooled across the whole city. Mayor and vice mayor are always citywide.
        keys = ['region', 'province', 'city']
        if position == 'COUNCILOR':
            keys = keys + ['district']

        for _, group in position_df.groupby(keys, dropna=False):
