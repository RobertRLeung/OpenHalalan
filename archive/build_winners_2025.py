...
def load_2019_reference_counts():
    ...
    """
    dynasty_df = pd.read_csv(SOURCE_WINNERS)
...
def main():
    ...
    all_data = []
    csv_files = list(raw_dir(2025).rglob("*.csv"))
    ...
    for position in ['MAYOR', 'VICE MAYOR', 'COUNCILOR']:
        ...
        # each of its 6 council districts), so seats are filled per district rather than
        # pooled across the city. Mayor and vice mayor are always citywide.
        keys = ['province', 'city']
        if position == 'COUNCILOR':
            keys = keys + ['district']

        for _, group in position_df.groupby(keys, dropna=False):
            # Winners are decided by VOTES. COMELEC's `rank` column is an alphabetical
            # index, not a vote standing - never sort on it.
